...
// applyGlobalEnvOverrides applies global LINEAR_URL, LINEAR_TOKEN
// environment variables over any config values.
func applyGlobalEnvOverrides(cfg *Config) {
	if v, ok := os.LookupEnv("LINEAR_URL"); ok {
		cfg.URL = v
	}
...
// (LINEAR_<UPPER(name)>_URL, _TOKEN) and overwrites the corresponding
// struct fields when set.
func applyEnvOverrides(cfg *Config) {
	if cfg.Name == "" {
		return
	}
	prefix := "LINEAR_" + strings.ToUpper(cfg.Name) + "_"
