...
export function descEditInputEnabled(state: string): boolean {
...
export function descEditApplyEnabled(state: string, proposal: string | undefined): boolean {
...
export function buildDescriptionPreview(saved: string, proposal: string | undefined, state: string): DescEditPreview {
  if (state === "applied") {
...
export function descEditAllowedFor(queue: string, bug = false, security = false, justPromoted = false): boolean {
  if (bug || security) return false;
