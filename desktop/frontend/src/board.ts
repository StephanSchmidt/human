...
function renderCard(card: Card): HTMLElement {
  ...
  if (card.tbaCount) {
    ...
    meta.push(
      `<span class="badge tba" title="${escapeAttr(`${n} question${n === 1 ? "" : "s"} the draft could not answer — open the card to answer them.`)}">${n} TBA</span>`,
    );
  }
...
async function promoteIdeaToBacklog(key: string): Promise<void> {
  const card = current.cards.find((c) => c.key === key);
  if (!card) return;
  try {
...
// (openDescEditModal's abandoned-Start branch), because the session the close
// handler could not see is still a session the daemon is holding. Nothing is
// ever written to the tracker before the user clicks Apply.
...
async function openDescEditModal(card: Card, opts: { promoted?: boolean } = {}): Promise<void> {
  ...
  if (!descEditAllowedFor(queueOf(card), card.bug, card.security, opts.promoted)) return;
  descEditCard = card;
  ...
  try {
    ...
    // session outlived the modal. Discard it here instead of adopting it into
    // state the closed modal no longer owns.
    if (descEditCard?.key !== card.key) {
      discardDescEditSession(started.state, started.sessionId);
      return;
    }
    descEdit = started;
  } catch (err) {
...
// discardDescEditSession ends a daemon-side session no modal owns any more.
// Fire-and-forget: nothing on screen waits on the result, and a failed discard
// only risks one stale session reattaching rather than corrupting anything
// (Discard never touches the tracker). Shared by the close handler and the
// abandoned-Start path so both abandonments end the session the same way.
function discardDescEditSession(state: string, sessionId: string | undefined): void {
  if (!descEditShouldDiscardOnClose(state, sessionId)) return;
  void go()
...
// registry, so there is no per-step backend round-trip: only the final
// scaffold call goes to Go.
